...
void m_state_machine_task(void *arg) {
  while (1) {
    ...
    case STATE_WIFI_CONNECT: {
      ...
      ble_wifi_get_credentials(ssid, pass);

      ESP_LOGI(TAG, "Ket noi Wi-Fi voi SSID: [%s]", ssid);
      wifi_init();

      app_wifi_config_t sta_cfg = {.max_retry = 3, .retry_delay_ms = 2000};
      strncpy(sta_cfg.ssid, ssid, sizeof(sta_cfg.ssid) - 1);
      strncpy(sta_cfg.password, pass, sizeof(sta_cfg.password) - 1);
      ...
      if (wifi_wait_for_connected(pdMS_TO_TICKS(15000))) {
        ...
        if (strlen(ssid) > 0) {
          esp_err_t err = nvs_save_wifi_credentials(ssid, pass);
          if (err == ESP_OK) {
